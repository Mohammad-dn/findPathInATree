...
function check(
  ...
  try {
    const actual = findPath(tree, id)?.map((node:any) => node.id);
    const check = isEqual(actual, expected);
    // const node=findPath(id,tree)
    if (!check) {
...
function _searchTree(nodeId:any, parent:any) {
  const stack = [[parent, []]];
  while (stack.length) {
     ...
     if (node.child) {
         stack.push(...node.child.map((node: any, i: any) => [node, [...path, i]]));
     }
  }
}
const path = _searchTree(49, deepTree);
